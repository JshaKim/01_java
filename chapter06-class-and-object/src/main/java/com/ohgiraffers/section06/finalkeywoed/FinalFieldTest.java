package com.ohgiraffers.section06.finalkeywoed;

public class FinalFieldTest {

    /*
    * final
    * final은 종단의 의미를 가진 키워드이다.
    * final 키워드를 사용할 수 있는 위치는 다양한 편이며 의미가 약간은 다을지만 결국 변경 불가의 의미를 가짐.
    * 1. 지역변수 : 초기화 이후 값 변결 불가
    * 2. 매개변수 : 호출시 전달한 인자 변경 불가
    * 3. 전역변수 : 인스턴스 생성 후 초기화 이후에 값 변경 불가
    * 4. 클래스(static) 변수 : 프로그램 starat 이후 값 변경 불가능
    * 5. non-static 메소드 : 메소드 재작성(overriding) 불가
    * 6. static 메소드 : 메소드 재작성(overriding) 불가
    * 7. 클래스 : 상속 불가능
    * */
    // 아래와 같이 상수 필드의 값을 할당하지 않으면 초기값이 0으로 고정되어 에러가 발생한다.
//    private final int NON_FINAL_NUM;

    private final int NON_FINAL_NUM = 10;
    ...
    * */
    ...
    private static double STATIC_DOUBLE; // 0.0일 경우 static은 클래스의 기본값을 의미한다.
    ...
    * */
    ...
    // 클래스의 일반 상수는 생성자를 토앟여 초기값을 전달하는 것이 가능하다.
    //private final String STATIC_STRING;
    // 상수 필드는 꼭 초기화를 해야하는 규칙을 가지고 있다.
    // 그러나 아래와 같이 생성을 하게되면 초기값이 전달이 안되고
